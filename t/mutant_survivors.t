...
subtest 'supported_languages() list context: defined, truthy, correct content' => sub {
	...
	is(scalar @langs, 6, 'Exactly six supported languages');
	is_deeply([sort @langs], [qw(de en es fa fr la)], 'Languages are de, en, fr, la, es, fa');
};
...
subtest 'supported_languages() scalar context: defined arrayref with content' => sub {
	...
	is_deeply([sort @{$ref}], [qw(de en es fa fr la)], 'Arrayref content is de, en, fr, la, es, fa');
};
